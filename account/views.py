...
from pages.models import FavoriteArticle, CommentAuthenticated, Articles
...
def show_profile_view(request):
    if request.user.is_authenticated:
        ...
        favorite_count = FavoriteArticle.objects.filter(author=user).count()
        comment_count = CommentAuthenticated.objects.filter(user=user).count()
        article_count = Articles.objects.filter(author=user).count()
        context = {
            'profile': profile,
            'title': 'Мой профиль',
